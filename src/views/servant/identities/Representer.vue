        ...
        <div class="upload-div" v-else>
          ...
          <div v-else>
            <FileUpload
              ...
              :auto="true"
              icon="pi pi-upload"
              chooseLabel="بارگذاری تصویر معرفی نامه"
    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        ...
        <Button
          ...
          label="ثبت"
          @click="updateRepresenter()"
          :disabled="!image || !selectedCompany || image.indexOf('blob:') > -1 || status"
          :loading="loading"
          v-else-if="!status"
        />
      </div>
...
export default {
  ...
  methods: {
    updateRepresenter() {
      ...
      this.identityService.updateRepresenter(
        data,
        () => {
          this.$toast.add({
            severity: "success",
            summary: "موفق",
            detail: "معرفی نامه با موفقیت ثبت شد.",
            life: 3000,
          });
          this.company = this.companies.filter((e) => e.id = data.company)[0];
