  ...
  <Card class="w-full">
    <template #title>
      <div class="flex justify-between">
        ...
        <div v-else>گواهی عدم سوء پیشینه</div>
      ...
      <div class="flex flex-col gap-12 justify-between">
        ...
        <div class="upload-div" v-else>
          ...
          <div v-else>
            <FileUpload
              ...
              :auto="true"
              icon="pi pi-upload"
              chooseLabel="بارگذاری تصویر گواهی"
        ...
        <div class="flex flex-col gap-2">
          ...
          <InputText
            v-else
            v-model="number"
            placeholder="شماره گواهی را وارد نمایید"
            class="w-full"
            :class="{ 'p-invalid': errors.number }"
            :disabled="loading"
          />
          <p class="text-xs text-red-600">{{ errors.number }}</p>
        </div>
      </div>
    </template>
    <template #footer>
      <div class="flex justify-end gap-3">
        ...
        <Button
          ...
          label="ثبت"
          @click="updateNoCriminalRecord()"
          :disabled="!image || !number || image.indexOf('blob:') > -1  || status"
          :loading="loading"
          v-else-if="!status"
        />
      </div>
...
export default {
  ...
  methods: {
    updateNoCriminalRecord() {
      this.loading = true;
      const data = {
        image: this.image,
        number: this.number,
      };
      this.identityService.updateNoCriminalRecord(
        data,
        () => {
          this.$toast.add({
            severity: "success",
            summary: "موفق",
            detail: "گواهی عدم سوء پیشینه با موفقیت ثبت شد.",
            life: 3000,
          });
