  ...
  <div class="pending flex flex-col gap-8">
    ...
    >
      ...
      <Column class="w-1/6" :header="fetch ? '' : 'جزئیات'">
        ...
        <template #body="{ data }">
          <div class="flex gap-4 items-center justify-center" v-if="data.tag">
            <Button
    ...
    <DetailsDialog ref="detailsDialog" @call="getList()" :reasons="reasons"/>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      statuses: {
        accepting: { severity: "warn", title: "در انتظار تایید" },
  ...
  methods: {
    getList() {
      ...
      this.serviceRequestsService.getList(
        ["accepting", "paying", "coordinating","verifying"],
        (data) => {
    ...
    confirmRequest() {
      // let index = null;
      // this.serviceRequestsList.forEach((element, i) => {
      //   if (element.id == id) {
      //     index = i;
      //   }
      // });
