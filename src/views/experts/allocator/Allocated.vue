<template>
  <div class="past flex flex-col gap-8">
    <DataTable
      ...
      dataKey="id"
      :paginator="serviceRequests.length > 10"
      :rows="10"
    ...
    >
      ...
      <Column header="نشانی" class="w-1/6 trow"
        ><template #header>
          <Skeleton v-if="fetching" />
          <p v-else>نشانی</p>
        </template>
      ...
      <Column header="خدمت" class="w-1/6 trow"
        ><template #header>
          <Skeleton v-if="fetching" />
          <p v-else>خدمت</p>
        </template>
      ...
      <Column header="تاریخ" class="w-1/6 trow"
        ><template #header>
          <Skeleton v-if="fetching" />
          <p v-else>تاریخ</p>
        </template>
      ...
      <Column class="w-1/6 trow" header="جزئیات"
        ><template #header>
          <Skeleton v-if="fetching" />
          <p v-else>جزئیات</p>
        </template>
...
export default {
  ...
  data() {
    return {
      ...
      statuses: {
        accepting: { title: "درانتظار خدمت دهنده", severity: "warning" },
        paying: { title: "درانتظار پرداخت", severity: "info" },
        coordinating: { title: "درانتظار هماهنگی", severity: "success" },
        verifying: { severity: "primary", title: "در انتظار صدور مجوز" },
        publishing: { title: "منتشر شده", severity: "warning" },
        doing: { title: "درانتظار انجام", severity: "scondary" },
        rating: { title: "درانتظار امتیازدهی", severity: "help" },
      },
      fetching: true,
    };
  },
  ...
  mounted() {
    let status = "accepting,paying,coordinating,doing,verifying,rating";
    const role = this.$route.path.split("/")[1];
    if (role == "flight") {
      status = "coordinating,doing,rating";
    }
